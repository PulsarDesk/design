...
/** Every glyph name in the Pulsar icon set. */
export type IconName =
  | 'connect' | 'devices' | 'gaming' | 'settings' | 'monitor' | 'lock' | 'search'
  | 'plus' | 'minus' | 'dots' | 'chevron' | 'chevronDown' | 'refresh' | 'expand'
  | 'keyboard' | 'clipboard' | 'power' | 'file' | 'chat' | 'mic' | 'shield' | 'bolt'
  | 'plug' | 'user' | 'star' | 'x' | 'check' | 'copy' | 'arrowRight' | 'wifi'
  | 'sun' | 'speaker';
...
export interface IconProps {
  ...
  /** Stroke width. */
  stroke?: number;
  /** Extra inline styles (e.g. color, fill). */
  style?: CSSProperties;
}
...
/**
 * Pulsar's line-icon set. A single stroked SVG, `currentColor` by default so it
 * inherits text color. Used throughout the app for navigation, buttons and status.
 */
export function Icon({ name, size = 20, stroke = 2, style }: IconProps) {
  return (
    <svg
      width={size}
      height={size}
