...
export interface GamingScreenProps {
  /** The host's game library. */
  games?: Game[];
  /** Called with the selected game when the stream is started. */
  onStream?: (game: Game) => void;
}

/**
 * The Gaming host screen: turn this machine into a stream host. A three-column
 * cover-art library (with HDR badges and a "whole desktop" tile) on the left;
 * a sticky config card on the right for resolution, framerate, bitrate, codec
 * (H.265/AV1) and HDR, plus a controller-status chip and a "Akışı başlat" CTA.
 */
export function GamingScreen({ games = GAMES, onStream }: GamingScreenProps) {
  const [sel, setSel] = useState<Game>(games[0]);
  ...
  return (
    <div>
      <ScreenHeader
        title="Gaming"
        sub="Bu bilgisayarı host yap ve oyunlarını her cihaza yayınla."
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 332px', gap: 18, alignItems: 'start' }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 13 }}>
            ...
            <Button variant="quiet" icon="plus" style={{ padding: '6px 10px', fontSize: 13 }}>
              Oyun ekle
            </Button>
          </div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    boxShadow: active ? 'var(--shadow)' : 'var(--shadow-xs)',
                    textAlign: 'left',
                  ...
                  <div style={{ height: 96, position: 'relative', background: `linear-gradient(140deg, ${g.g[0]}, ${g.g[1]})` }}>
                    ...
                    {g.desktop && (
                      <div style={{ position: 'absolute', top: 8, left: 8 }}>
                        <Icon name="monitor" size={18} style={{ color: '#fff', opacity: 0.9 }} />
                      </div>
                    )}
                    {g.hdr && (
                      <span style={{ position: 'absolute', top: 8, right: 8, fontFamily: 'var(--font-mono)', fontSize: 9, fontWeight: 700, color: '#fff', background: 'oklch(0 0 0 / 0.3)', padding: '2px 6px', borderRadius: 5 }}>
                        HDR
                      </span>
                    )}
                  </div>
        ...
        <Card pad={22} style={{ position: 'sticky', top: 0 }}>
          <div style={{ height: 110, borderRadius: 'var(--r)', marginBottom: 16, position: 'relative', overflow: 'hidden', background: `linear-gradient(140deg, ${sel.g[0]}, ${sel.g[1]})` }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
            <div>
              ...
              <Seg value={res} onChange={(v) => setRes(String(v))} options={['1080p', '1440p', '4K']} />
            </div>
            <div>
              <div className="cfg-lab">Kare hızı</div>
              <Seg value={fps} onChange={(v) => setFps(Number(v))} options={[{ value: 60, label: '60' }, { value: 120, label: '120' }, { value: 144, label: '144' }]} />
            </div>
            <div>
            ...
            <div>
              ...
              <Seg value={codec} onChange={(v) => setCodec(String(v))} options={[{ value: 'h265', label: 'H.265' }, { value: 'av1', label: 'AV1' }]} />
            </div>
            ...
            <Button variant="primary" icon="bolt" onClick={() => onStream?.(sel)} style={{ justifyContent: 'center', marginTop: 2 }}>
              Akışı başlat
            </Button>
          </div>
        </Card>
